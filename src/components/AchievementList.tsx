        ...
        {Object.entries(grouped).map(([date, items], groupIndex) => (
          ...
          >
            <div className="sticky top-0 bg-white/80 backdrop-blur-md z-10 py-1 mb-2 border-b border-neutral-100 flex items-baseline justify-between px-0.5">
              <h2 className="text-[9.5px] font-mono lowercase tracking-[0.05em] text-black font-semibold">
                {format(new Date(date), 'MMM dd, yyyy')}
              </h2>
              <span className="text-[6.5px] font-mono text-neutral-300 uppercase tracking-[0.2em] font-normal">{items.length} units</span>
            </div>
            ...
            <div className="space-y-1">
              {items.map((item, i) => (
                ...
                >
                  <div className="flex items-center gap-1.5 w-10 shrink-0 py-1 opacity-40 group-hover:opacity-100 transition-opacity">
                    <div className="w-[0.5px] h-3 bg-black" />
                    <span className="text-[7.5px] font-mono text-black font-medium tracking-tighter">
                      {item.timestamp ? format(item.timestamp.toDate(), 'HH:mm') : '--:--'}
                    </span>
                  </div>
                  
                  <div className="flex-1 flex items-start justify-between gap-4 py-0.5 min-h-[1.5rem]">
                    {editingId === item.id ? (
                      <form onSubmit={handleUpdate} className="flex-1 flex flex-col gap-2">
                        <textarea
                          value={editValue}
                          onChange={(e) => setEditValue(e.target.value)}
                          className="w-full text-[10px] font-sans font-medium leading-tight tracking-tight text-black bg-neutral-50/50 p-2 border-l border-black focus:ring-0 resize-none min-h-[3rem]"
                          autoFocus
                          onKeyDown={(e) => {
                          ...
                          <div className="flex gap-1.5">
                            {(['work', 'growth', 'personal', 'other'] as const).map(t => (
                              <button
                                ...
                                className={`text-[6.5px] font-mono lowercase tracking-widest px-1.5 py-0.5 border ${editType === t ? 'bg-black text-white border-black' : 'text-neutral-300 border-neutral-100 hover:border-neutral-200'}`}
                              >
                                {t}
                              </button>
                            ))}
                      ...
                      <div className="flex items-center gap-4">
                        <span className="text-[8px] font-mono text-axiom-warning lowercase italic shrink-0 tracking-tighter">confirm_deletion?</span>
                        <div className="flex items-center gap-2.5">
                          <button 
                            onClick={() => item.id && handleDelete(item.id)}
                            className="text-[8px] font-mono text-axiom-warning hover:bg-axiom-warning hover:text-white px-1.5 transition-colors lowercase border border-axiom-warning/10"
                          >
                            yes
                          </button>
                          <button 
                            onClick={() => setConfirmDelete(null)}
                            className="text-[8px] font-mono text-neutral-400 hover:text-black px-1.5 transition-colors lowercase border border-neutral-100"
                          >
                            no
                          </button>
                        </div>
                      ...
                      <>
                        <div className="flex-1 flex flex-col gap-0.5 max-w-[280px]">
                          <p className="text-[10px] font-sans font-medium leading-tight tracking-tight text-black flex-1">
                            {item.text}
                          </p>
                          <span className={`text-[5.5px] font-mono lowercase tracking-[0.1em] opacity-40 transition-opacity font-bold group-hover:opacity-100 ${getTypeColor(item.type)}`}>
                            {item.type}
                          </span>
                        </div>
                        
                        {isOwner && (
                          <div className="flex items-center opacity-0 group-hover:opacity-100 transition-opacity gap-0.5 pt-0.5">
                            <button 
